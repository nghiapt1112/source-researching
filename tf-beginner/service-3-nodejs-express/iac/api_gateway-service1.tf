resource "aws_api_gateway_resource" "proxy-service1" {
...
resource "aws_api_gateway_method" "proxy-method-service1" {
  rest_api_id   = "${aws_api_gateway_rest_api.nathan-apigw.id}"
  resource_id   = "${aws_api_gateway_resource.proxy-service1.id}"
  http_method   = "ANY"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "lambda-service1" {
  rest_api_id = "${aws_api_gateway_rest_api.nathan-apigw.id}"
  resource_id = "${aws_api_gateway_method.proxy-method-service1.resource_id}"
  http_method = "${aws_api_gateway_method.proxy-method-service1.http_method}"
